...
var uploadToQiniu = require('./qiniu').uploadToQiniu;
'use strict';
var port;
var hostname;
...
function upload(receiver, to, release, content, subpath, callback){
    // 如果读取到七牛标记，则部署文件到服务器
    if(receiver === "http://qiniu"){
        var filename = release.replace(/^\//,"");
        uploadToQiniu(filename,content,function(err,ret){
            if(err){
                fis.log.error('upload file [' + subpath + '] to [qiniu] error. [' + err.code + ']' + err.error);
            }else{
                callback();
            }
        });
