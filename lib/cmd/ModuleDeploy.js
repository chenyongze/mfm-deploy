...
(function() {
  ...
  ModuleDeploy = (function(_super) {
    ...
    ModuleDeploy.prototype.templatesServerConfig_ = function(cb) {
      var body, channelId, deployContent, deployFilename, error, response, statics_output, statics_receiver, uname, work_dir,
        _this = this;
      if (!this.cmd.deploy) {
    ...
    ModuleDeploy.prototype.prepareComplete = function(cb) {
      ...
      assert.func(cb);
      if (!this.cmd.deploy) {
        return cb("布署标识未指定 -d <name>");
      }
      if (this.cmd.pre_release && this.cmd.release) {
        return this.next("不能同时使用--pre_release 和--release参数");
      }
      if (this.cmd.release || this.cmd.pre_release) {
        if (!this.cmd.channel) {
          return this.next("请填写要布署到的通道编号:例如12");
        }
      }
      if (this.cmd.release) {
      ...
      } else if (this.cmd.pre_release) {
        utils.ask_if_v(("与代码同步上线:将打包模块【" + this.module + "】及相关资源到线上,并生成模板布署id,请确认操作").yellow, function() {
          err = arguments[0], answer = arguments[1];
          _$$_2();
        });
        ...
        utils.ask_if_v(("警告:将要以【" + this.cmd.deploy + "】规则,布署模块【" + this.module + "】及相关资源到线上,模块和布署标识是否正确").red, function() {
          err = arguments[0], answer = arguments[1];
          _$$_2();
        });
      }
      function _$$_2() {
        var _i, _len, _ref1;
        if (!answer) {
          return _this.next("模块【" + _this.module + "】的布署操作取消");
        }
        cmds = [];
        ...
        utils.work_dir_shell2(["./release.sh"], function() {
